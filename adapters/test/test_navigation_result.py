...
@pytest.mark.parametrize("result", [
    NS(error_msg="Transform unavailable", error_code=102, FAILED_TO_MAKE_PROGRESS=105),
...
def test_existing_diagnostic_is_preserved_without_duplication():
    result = NS(error_msg="Failed to make progress", message="Failed to make progress",
                error_code=105, FAILED_TO_MAKE_PROGRESS=105)
    assert navigation_failure_reason(result) == "Failed to make progress; error_code=105"
