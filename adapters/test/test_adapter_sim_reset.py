...
#
# The adapter requests the simulation's single bounded lifecycle owner. A reply
# is not success unless that owner confirmed every node active.
...
def nav_bridge(sim, ready):
    ...
    bridge.path_client = MagicMock()
    bridge.path_client.server_is_ready.return_value = ready
    bridge._nav_down_since = 0.0
    bridge._nav_recovered_at = 0.0
    bridge._lifecycle_service_response = MagicMock(
        return_value=types.SimpleNamespace(success=True, message="")
    )
    return bridge
...
def test_a_healthy_nav2_is_left_alone(sim):
    ...
    bridge._lifecycle_service_response.assert_not_called()
...
def test_recovery_is_not_hammered(sim, monkeypatch):
    ...
    assert bridge._lifecycle_service_response.call_count == 1
    ...
    clock[0] += 2
    assert bridge.recover_nav_if_down() is True
    assert bridge._lifecycle_service_response.call_count == 2
...
def test_failed_lifecycle_recovery_is_not_reported_as_success(sim, monkeypatch):
    """Silence here would leave a robot that cannot navigate and cannot say why."""
    bridge = nav_bridge(sim, ready=False)
    bridge._lifecycle_service_response.return_value = types.SimpleNamespace(
        success=False, message="controller remained inactive"
    )
    clock = [1000.0]
    monkeypatch.setattr(sim.time, "monotonic", lambda: clock[0])
