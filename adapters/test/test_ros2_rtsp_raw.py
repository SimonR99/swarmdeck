...
def test_raw_callback_skips_pixel_conversion_when_fps_gate_closed(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "sensor_msgs.msg", NS(CompressedImage=object, Image=object))
    ...
    publisher = NS(_failed=NS(is_set=lambda: False), _last_compressed_at=0.0,
                   raw_source=object(), _can_push=lambda source: False)
...
def test_compressed_preference_timeout_is_named_and_raw_fallback_resumes(monkeypatch):
    ...
    publisher = NS(_failed=NS(is_set=lambda: False), _last_compressed_at=100.0,
                   _can_push=lambda source: True, raw_source=object())
...
def test_raw_callback_pushes_rgb_directly_to_raw_input(monkeypatch):
    media = Path(__file__).resolve().parents[1] / "media" / "ros2_rtsp.py"
    gi = NS(require_version=lambda *args: None,
    ...
    pushed = Mock()
    publisher = NS(_failed=NS(is_set=lambda: False), _last_compressed_at=0.0,
                   _can_push=lambda source: True, raw_source=source, _raw_caps=None,
                   _frame_period_s=0.1, _push_frame=pushed)
