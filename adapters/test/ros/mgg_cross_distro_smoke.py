...
def main():
    ...
    if sys.argv[1] == 'planner':
        ...
        process = subprocess.Popen([
            'ros2', 'run', 'mgg_pci', 'mgg_pci_node', '--ros-args',
            '-r', '__ns:=/probe/mgg', '-p', 'world_frame:=map',
        ])
        ...
        bridge = SimpleNamespace(
            node=node, id='probe', cfg={'link_timeout_s': 60}, map_frame='map',
            navigate_to=goals.append, cancel_goal=lambda: stops.append(True),
            drive=lambda *_: None,
        )
        ...
        try:
            wait_until(lambda: explorer.start_client.service_is_ready()
                       and explorer.stop_client.service_is_ready())
            explorer.start()
            wait_until(lambda: bool(goals) and explorer.pending is None)
            assert explorer.active and goals[-1] == {'x': 2., 'y': 1., 'yaw': 0.}
            explorer.stop()
