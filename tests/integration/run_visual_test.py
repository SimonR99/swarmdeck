...
def capture(sock_path: str, ticks: int):
    ...
    for _ in range(ticks):
        ...
        if magic != OBSERVATION_MAGIC:
            raise RuntimeError(
                f"bad observation magic {magic!r}; the loop "
        ...
        for _r in range(count):
            ...
            if struct.unpack("<B", recv_exact(client, 1))[0]:  # camera
                ...
                rgb = np.frombuffer(
                    recv_exact(client, w * h * 3), dtype=np.uint8
                ).reshape((h, w, 3))
                rgbs[rid] = rgb
                if struct.unpack("<B", recv_exact(client, 1))[0]:
                    depths[rid] = np.frombuffer(
                        recv_exact(client, w * h * 4), dtype=np.float32
                    ).reshape((h, w))
    ...
    print(
        f"[visual] {ticks} exchanges, {sim_seconds:.1f}s simulated in "
...
def write_outputs(outdir: Path, rgbs, depths, scans, poses) -> int:
    ...
    for rid in ids:
        ...
        if rgb is None:
            ...
            if int(rgb.max()) < 8:
                failures.append(
                    f"{rid}: camera frame is black " f"(max channel {int(rgb.max())})"
                )
            row.append(rgb)
        ...
        if not pts:
            ...
            if near > len(pts) * 0.25:
                failures.append(
                    f"{rid}: {near}/{len(pts)} lidar hits within "
    ...
    if tiles:
        ...
        board = np.zeros((sum(t.shape[0] for t in tiles), width, 3), dtype=np.uint8)
        y = 0
        for tile in tiles:
    ...
    for rid in ids:
        if rid in poses:
            x, y, z = poses[rid]
            print(
                f"[visual] {rid} at ({x:+.2f}, {y:+.2f}, {z:+.2f}) "
...
def main() -> int:
    ...
    ap.add_argument(
        "--argos-build",
        default=os.environ.get("SWARMDECK_ARGOS_BUILD", str(REPO / "argos" / "build")),
        help="Directory holding libswarmdeck_argos.so",
    )
    ap.add_argument(
        "--argos-root",
        default=os.environ.get("ARGOS_ROOT", str(Path.home() / "Projects" / "argos3")),
    ...
    cfg_path = (
        Path(args.config) if Path(args.config).is_absolute() else REPO / args.config
    )
    import yaml
    ...
    proc = subprocess.Popen(["argos3", "-c", str(argos_file)], cwd=str(rundir), env=env)
    try:
