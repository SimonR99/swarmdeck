...
def _render_component(
    ...
    for origin_xy, points_world in contributions:
        ...
        hit_counts[min_gy : max_gy + 1, min_gx : max_gx + 1] += counts.reshape(
            lh, lw
        ).astype(np.int32)
...
def _rasterize_free(
    ...
    while start < order.shape[0]:
        ...
        grid_x = np.floor(np.float32(origin_x) + step_index * step_dx[None, :]).astype(
            np.int32
        )
        grid_y = np.floor(np.float32(origin_y) + step_index * step_dy[None, :]).astype(
            np.int32
        )
        valid &= (
            (grid_x >= 0)
        ...
        if valid.any():
            ...
            counts = np.bincount(flat, minlength=lw * lh)
            free[min_gy : max_gy + 1, min_gx : max_gx + 1] += counts.reshape(
                lh, lw
            ).astype(np.int32)
