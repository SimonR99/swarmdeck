...
def _hinted_graph() -> (
    tuple[CollaborativeBackend, list[Keyframe], OptimizedGraph, np.ndarray, np.ndarray]
):
...
def test_world_hint_keeps_the_graph_it_regauges() -> None:
    ...
    assert not np.allclose(gauged.poses[included[1].id], hint @ included[1].t_odom_base)
...
def test_world_hint_leaves_the_solver_output_alone() -> None:
    ...
    assert np.allclose(
        optimized.t_world_trajectory[TrajectoryId("alpha")], solver_frame
    )
