...
def test_parse_timestamped_plan_lines_epoch():
    ...
    expected = dt.datetime(
        2026, 9, 23, 12, 4, 1, 123456, tzinfo=dt.timezone.utc
    ).timestamp()
...
def test_latency_window_filter_uses_probe_window(monkeypatch):
    result = _trace(
        monkeypatch,
        [_pose(2049, 0), _pose(2051, 0.2)],
        plans=(1000, 2050, 2200),
        start=2000,
        end=2100,
    )
...
def test_latency_window_filter_empty_when_all_outside(monkeypatch):
    result = _trace(
        monkeypatch, [_pose(2050, 0)], plans=(1000, 9000), start=2000, end=2100
    )
...
def test_ref_pos_uses_last_pre_plan_sample(monkeypatch):
    result = _trace(
        monkeypatch, [_pose(1, 0), _pose(9, 1), _pose(11, 1.01), _pose(12, 1.2)]
    )
...
def test_latency_superseded_plan_cannot_borrow_later_motion(monkeypatch):
    result = _trace(
        monkeypatch, [_pose(9, 0), _pose(19, 0), _pose(21, 0.2)], plans=(10, 20)
    )
    assert result["latency_s"] == {"robot_0": [1]}
...
def test_latency_no_reset_spanning_reference_for_a_new_plan(monkeypatch):
    result = _trace(
        monkeypatch,
        [_pose(9, 0), {"utc": 11, "reset": ["robot_0"]}, _pose(21, 1), _pose(22, 1.2)],
        plans=(20,),
    )
    assert result["latency_s"] == {"robot_0": [2]}
...
def test_latency_displacement_exactly_at_next_plan_is_cut_off(monkeypatch):
    result = _trace(monkeypatch, [_pose(9, 0), _pose(20, 0.2)], plans=(10, 20))
    assert result["latency_s"] == {}
    assert result["cut_off_plan_count"] == 1
...
def test_short_command_py_script():
    assert (
        short_command(
            "python3 /opt/ros/jazzy/lib/swarmdeck_sim/swarmdeck_argos_bridge.py __ns:=/sim"
        )
    ...
    results = iter(
        (
            ...
            subprocess.CompletedProcess(
                [],
                0,
                stdout="\n".join(_make_ts_line(t, _PLAN_CONTENT) for t in plans),
                stderr="",
            ),
        )
    )
...
def test_latency_registration_only_change_is_not_motion(monkeypatch):
    result = _trace(
        ...
        [
            ...
            _pose(11, 0.2, transform={"x": 0.2, "y": 0, "yaw": 0}),
            _pose(12, 0.4, transform={"x": 0.2, "y": 0, "yaw": 0}),
        ],
    )
    assert result["latency_s"] == {}
...
def test_latency_real_motion_in_navigation_frame(monkeypatch):
    ...
    result = _trace(
        monkeypatch, [_pose(9, 5, 4, transform), _pose(11, 5, 4.2, transform)]
    )
    assert result["latency_s"] == {"robot_0": [1]}
...
def test_browser_report_prints_nproc(monkeypatch):
    ...
    monkeypatch.setattr(
        profile_stack, "real_time_factor", lambda *a: {"error": "offline"}
    )
    ...
    monkeypatch.setattr(
        profile_stack, "browser_cpu", lambda *a, **kw: {**metrics, "nproc": 20}
    )
...
def test_browser_missing_nproc_is_unavailable_not_a_crash(monkeypatch, tmp_path):
    ...
    assert (
        "nproc unavailable"
        in profile_stack.browser_cpu("http://localhost:5173")["error"]
    )
