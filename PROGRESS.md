...
| **Collaborative SLAM** | `:8090` / `slam/` | Python 3.12 / GTSAM pose graph optimizer, isotropic loop closure weighting, probabilistic ray clearing (`render.py`), and scoped grid publisher. |
...
2. **Real-Time SLAM Optimization**:
   - Every single incoming keyframe triggers graph optimization (`OPTIMIZE_EVERY_N = 1`).
   - Occupancy grid rasterization uses log-odds ray clearing (+3 hits, -1 misses) to remove transient dynamic clutter.
   - Output scopes (`component:<id>`, `robot:<id>`) are published to the server and pushed live via WebSocket.
...
```bash
# Run server and adapter test suite (438 tests)
./server/.venv/bin/pytest server/tests adapters/test

# Run SLAM test suite (102 tests)
