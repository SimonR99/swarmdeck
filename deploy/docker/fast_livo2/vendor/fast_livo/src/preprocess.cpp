...
// Livox layout delivered as PointCloud2: x, y, z, intensity, timestamp (double),
// tag, line. Written for the ARGoS simulation, which publishes exactly this and
// which the Velodyne and Ouster handlers reject with
//   "Failed to find match for field 't'" / "field 'ring'"
// because pcl::fromROSMsg matches fields by name.
//
// FAST-LIVO2 wants curvature as the per-point time offset in milliseconds; the
// offset is taken against the first point so absolute-vs-relative and
// seconds-vs-nanoseconds stamping both work.
void Preprocess::livox_pcl2_handler(const sensor_msgs::PointCloud2::ConstSharedPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();

  pcl::PointCloud<livox_pcl2_ros::Point> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  const int plsize = pl_orig.points.size();
  if (plsize == 0) return;
  pl_surf.reserve(plsize);

  const double t0 = pl_orig.points[0].timestamp;
  double span = pl_orig.points[plsize - 1].timestamp - t0;
  // Seconds give a span of order 0.1 for a 10 Hz scan; nanoseconds give 1e8.
  const double to_ms = (std::fabs(span) > 1e3) ? 1e-6 : 1e3;
  ...
  for (int i = 0; i < plsize; i++)
  {
    ...
    if (range < blind_sqr) continue;
    if (!std::isfinite(src.x) || !std::isfinite(src.y) || !std::isfinite(src.z)) continue;
    ...
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.curvature = static_cast<float>((src.timestamp - t0) * to_ms);
    pl_surf.points.push_back(added_pt);
  }
}
