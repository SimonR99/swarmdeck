...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  const double &imu_beg_time = ros::toSec(v_imu.front()->header.stamp);
  const double &imu_end_time = ros::toSec(v_imu.back()->header.stamp);
  ...
  if (!imu_time_init)
  {
    // imu_time = ros::toSec(v_imu.front()->header.stamp) - first_lidar_time;
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      if (ros::toSec(tail->header.stamp) < prop_beg_time) continue;
      ...
      // #ifdef DEBUG_PRINT
      fout_imu << setw(10) << ros::toSec(head->header.stamp) - first_lidar_time << " " << angvel_avr.transpose() << " " << acc_avr.transpose() << endl;
      // #endif

      // imu_time = ros::toSec(head->header.stamp) - first_lidar_time;

      angvel_avr -= state_inout.bias_g;
      acc_avr = acc_avr * G_m_s2 / mean_acc.norm() - state_inout.bias_a;

      if (ros::toSec(head->header.stamp) < prop_beg_time)
      {
        // printf("00 \n");
        dt = ros::toSec(tail->header.stamp) - last_prop_end_time;
        offs_t = ros::toSec(tail->header.stamp) - prop_beg_time;
      }
      else if (i != v_imu.size() - 2)
      {
        // printf("11 \n");
        dt = ros::toSec(tail->header.stamp) - ros::toSec(head->header.stamp);
        offs_t = ros::toSec(tail->header.stamp) - prop_beg_time;
      }
      else
      {
        // printf("22 \n");
        dt = prop_end_time - ros::toSec(head->header.stamp);
      ...
      // cout<<setw(20)<<"offset_t: "<<offs_t<<"ros::toSec(tail->header.stamp):
      // "<<ros::toSec(tail->header.stamp)<<endl; printf("[ LIO Propagation ]
      // offs_t: %lf \n", offs_t);
