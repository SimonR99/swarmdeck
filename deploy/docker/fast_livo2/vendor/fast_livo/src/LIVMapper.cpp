...
void LIVMapper::readParameters(ros::NodeHandle &nh)
{
  ...
  nh.param<int>("vio/map_report_every", vio_map_report_every, 200);
...
void LIVMapper::initializeComponents() 
{
  ...
  // Upstream defaults these four to an empty vector and then indexes [0..8]
  // through VEC_FROM_ARRAY / MAT_FROM_ARRAY, so running without a config is a
  // straight out-of-bounds read: the node dies with SIGSEGV and no log line at
  // all. Say what is actually wrong instead.
  auto require = [](const char *name, const std::vector<double> &v, size_t n) {
    if (v.size() != n)
      throw std::runtime_error(
          std::string("extrin_calib/") + name + " must have " + std::to_string(n) +
          " entries, got " + std::to_string(v.size()) +
          ". Pass a config: --ros-args --params-file <config>.yaml (and note the "
          "file must be ROS 2 shaped, /**: ros__parameters: ...).");
  };
  require("extrinsic_T", extrinT, 3);
  require("extrinsic_R", extrinR, 9);
  require("Pcl", cameraextrinT, 3);
  require("Rcl", cameraextrinR, 9);
  ...
  // Upstream called vk::camera_loader::loadFromRosNs, which reads the camera
  // model off the ROS 1 parameter server. That is the only reason fast_livo
  // depended on vikit_ros. Build the pinhole model directly instead, keeping
  // vikit's own parameter names so existing configs and the ARGoS profile
  // generator both work unchanged:
  //   camera/projection_parameters : [fx, fy, cx, cy]
  //   camera/distortion_parameters : [k1, k2, p1, p2, k3]
  {
    ros::NodeHandle nh(ros::g_node);
    int cam_w = 1280, cam_h = 720;
    double cam_scale = 1.0;
    std::vector<double> proj, dist;
    nh.param<int>("camera/image_width", cam_w, cam_w);
    nh.param<int>("camera/image_height", cam_h, cam_h);
    nh.param<double>("camera/scale", cam_scale, cam_scale);
    nh.param<std::vector<double>>("camera/projection_parameters", proj, std::vector<double>());
    nh.param<std::vector<double>>("camera/distortion_parameters", dist, std::vector<double>());
    if (proj.size() != 4)
      throw std::runtime_error(
          "camera/projection_parameters must be [fx, fy, cx, cy], got " +
          std::to_string(proj.size()) + " entries");
    dist.resize(5, 0.0);
    vio_manager->cam = new vk::PinholeCamera(cam_w, cam_h, cam_scale,
  ...
  vio_manager->map_report_every = vio_map_report_every;
...
#else
  // Livox support is compiled out; lidar_type AVIA is unreachable here.
  if (p_pre->lidar_type == AVIA)
    throw std::runtime_error("lidar_type AVIA needs livox_ros_driver, which this "
                             "build omits. Botman is an Ouster: use lidar_type 3.");
  sub_pcl = nh.subscribe(lid_topic, 200000, &LIVMapper::standard_pcl_cbk, this);
#endif
  sub_imu = nh.subscribe(imu_topic, 200000, &LIVMapper::imu_cbk, this);
  // Pick the callback from the topic name: a ".../compressed" topic carries
  // CompressedImage, and subscribing to it as Image yields silence.
  if (img_topic.size() >= 10 && img_topic.compare(img_topic.size() - 10, 10, "compressed") == 0)
...
void LIVMapper::imu_prop_callback(const ros::TimerEvent &e)
{
  ...
  if (imu_prop_enable && !prop_imu_buffer.empty())
  {
    ...
    if (state_update_flg)
    {
      ...
      while ((!prop_imu_buffer.empty() && ros::toSec(prop_imu_buffer.front().header.stamp) < latest_ekf_time))
      {
      ...
      {
        double t_from_lidar_end_time = ros::toSec(prop_imu_buffer[i].header.stamp) - latest_ekf_time;
    ...
    else
    {
      ...
      double t_from_lidar_end_time = ros::toSec(newest_imu.header.stamp) - latest_ekf_time;
      double dt = t_from_lidar_end_time - last_t_from_lidar_end_time;
      prop_imu_once(imu_propagate, dt, acc_imu, omg_imu);
      last_t_from_lidar_end_time = t_from_lidar_end_time;
    }
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::PointCloud2::ConstSharedPtr &msg)
{
  ...
  // ROS_INFO("get point cloud at time: %.6f", ros::toSec(msg->header.stamp));
...
#endif
{
  ...
  // if ((abs(ros::toSec(msg->header.stamp) - last_timestamp_lidar) > 0.2 && last_timestamp_lidar > 0) || sync_jump_flag)
  // {
  //   ROS_WARN("lidar jumps %.3f\n", ros::toSec(msg->header.stamp) - last_timestamp_lidar);
  //   sync_jump_flag = true;
  //   msg->header.stamp = ros::Time().fromSec(last_timestamp_lidar + 0.1);
  // }
  if (abs(last_timestamp_imu - ros::toSec(msg->header.stamp)) > 1.0 && !imu_buffer.empty())
  {
    double timediff_imu_wrt_lidar = last_timestamp_imu - ros::toSec(msg->header.stamp);
  ...
  double cur_head_time = ros::toSec(msg->header.stamp);
...
void LIVMapper::imu_cbk(const sensor_msgs::Imu::ConstSharedPtr &msg_in)
{
  ...
  // ROS_INFO("get imu at time: %.6f", ros::toSec(msg_in->header.stamp));
  sensor_msgs::Imu::Ptr msg(new sensor_msgs::Imu(*msg_in));
  msg->header.stamp = ros::Time().fromSec(ros::toSec(msg->header.stamp) - imu_time_offset);
...
{
  if (!img_en) return;
  cv::Mat decoded = cv::imdecode(cv::Mat(msg_in->data), cv::IMREAD_COLOR);
  if (decoded.empty())
  {
    ROS_WARN("could not decode a CompressedImage on the image topic (format '%s')",
             msg_in->format.c_str());
    return;
  }
  auto raw = std::make_shared<sensor_msgs::msg::Image>();
  cv_bridge::CvImage bridge(msg_in->header, "bgr8", decoded);
  bridge.toImageMsg(*raw);
  img_cbk(sensor_msgs::msg::Image::ConstSharedPtr(raw));
}
...
void LIVMapper::img_cbk(const sensor_msgs::ImageConstPtr &msg_in)
{
  ...
  // if ((abs(ros::toSec(msg->header.stamp) - last_timestamp_img) > 0.2 && last_timestamp_img > 0) || sync_jump_flag)
  // {
  //   ROS_WARN("img jumps %.3f\n", ros::toSec(msg->header.stamp) - last_timestamp_img);
  //   sync_jump_flag = true;
  ...
  //   if (i % 4 != 0) return;
  // }
  // double msg_header_time =  ros::toSec(msg->header.stamp);
  double msg_header_time = ros::toSec(msg->header.stamp) + img_time_offset;
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case ONLY_LIO:
  {
    ...
    {
      if (ros::toSec(imu_buffer.front()->header.stamp) > meas.lidar_frame_end_time) break;
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      double imu_newest_time = ros::toSec(imu_buffer.back()->header.stamp);
      ...
      while (!imu_buffer.empty())
      {
        if (ros::toSec(imu_buffer.front()->header.stamp) > m.lio_time) break;

        if (ros::toSec(imu_buffer.front()->header.stamp) > meas.last_lio_update_time) m.imu.push_back(imu_buffer.front());

        imu_buffer.pop_front();
        // printf("[ Data Cut ] imu time: %lf \n",
        // ros::toSec(imu_buffer.front()->header.stamp));
      }
      mtx_buffer.unlock();
      sig_buffer.notify_all();
    ...
    case LIO:
    {
      ...
      // printf("[ Data Cut ] VIO \n");
      meas.measures.clear();
      double imu_time = ros::toSec(imu_buffer.front()->header.stamp);
      ...
      // {
      //   imu_time = ros::toSec(imu_buffer.front()->header.stamp);
...
void LIVMapper::publish_path(const ros::Publisher pubPath)
{
  ...
  // Upstream appends forever AND republishes the entire path every frame, so
  // both the retained message and the per-frame serialisation cost grow
  // without bound over a long deployment. Keep a trailing window instead.
  if (path_max_poses > 0 && (int)path.poses.size() > path_max_poses)
  {
    path.poses.erase(path.poses.begin(), path.poses.begin() + (path.poses.size() - (size_t)path_max_poses));
  }
