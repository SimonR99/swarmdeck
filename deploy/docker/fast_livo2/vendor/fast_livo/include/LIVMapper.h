...
public:
  ...
  // Botman's OAK-D publishes only /oak/rgb/image_raw/compressed. Upstream
  // subscribes as raw sensor_msgs/Image, and a type mismatch on a topic
  // delivers nothing without reporting anything, so the visual half would sit
  // silently idle. Decode and hand the same buffer to img_cbk.
