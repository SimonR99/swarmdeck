...
def hardware_settings(config):
    ...
    return {
        ...
        'RobotParams.center_offset': [(max(xs) + min(xs)) / 2,
                                      (max(ys) + min(ys)) / 2, 0.0],
...
def generate_launch_description():
    ...
    return LaunchDescription(module.robot_nodes(
        os.environ['SWARMDECK_ROBOT_ID'], config['map_frame'], topics['odom'],
        planner['cloud_topic'], '/tf', '/tf_static',
        os.environ.get('MGG_PARAMS_FILE', str(here / 'config/hardware.yaml')),
        False, planner_overrides=hardware_settings(config),
