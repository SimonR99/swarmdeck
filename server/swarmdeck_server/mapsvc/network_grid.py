...
class NetworkGridAccumulator:
    ...
    def _expand_to_cover(
        ...
        new_min_x = min(
            meta.origin_x, math.floor((min_x - EXPAND_MARGIN_M) / res) * res
        )
        new_min_y = min(
            meta.origin_y, math.floor((min_y - EXPAND_MARGIN_M) / res) * res
        )
    ...
    def integrate(self, x: float, y: float, quality_pct: float) -> bool:
        ...
        gx1 = min(
            meta.width, int(math.ceil((x + radius - meta.origin_x) / meta.resolution))
        )
        gy0 = max(0, int(math.floor((y - radius - meta.origin_y) / meta.resolution)))
        gy1 = min(
            meta.height, int(math.ceil((y + radius - meta.origin_y) / meta.resolution))
        )
        ...
        kernel = np.where(mask, np.maximum(0.08, 1.0 - distance / radius), 0.0).astype(
            np.float32
        )
        ...
        if overflow.any():
            scale = np.where(
                overflow, MAX_CELL_WEIGHT / np.maximum(target_weight, 1e-6), 1.0
            ).astype(np.float32)
            target_sum *= scale
            target_weight *= scale
        self.revision += 1
