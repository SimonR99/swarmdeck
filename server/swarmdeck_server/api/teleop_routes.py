    ...
    if duration > 0.0 and (linear != 0.0 or angular != 0.0):
        ...
        async def _auto_stop(rid: str, delay: float):
            ...
            await state.registry.send(
                rid,
                {"type": "drive", "linear": 0.0, "angular": 0.0, **state.stamps()},
            )
            state.events.log("agent_drive_stop", {"robot_id": rid})
...
async def post_robot_goal(robot_id: str, request: Request) -> Any:
    ...
    if error := robot_command_error(robot_id):
        return JSONResponse({"error": error}, status_code=409)
    if not state.registry.can(robot_id, "plan_objective"):
        return JSONResponse(
    ...
    if "z" in body:
        goal["z"] = float(body["z"])
    taken_by = state.goal_taken(goal, exclude=robot_id)
    if taken_by:
        return JSONResponse(
    ...
    state.events.log("agent_goal", {"robot_id": robot_id, "goal": goal})
...
async def post_robot_cancel(robot_id: str) -> Any:
    ...
    sent = await state.registry.send(
        robot_id, {"type": "cancel_goal", **state.stamps()}
    )
    if sent:
        robot.goal = None
    ...
    state.events.log("agent_cancel", {"robot_id": robot_id})
    return {"ok": True, "robot_id": robot_id}


async def post_robot_stop(robot_id: str) -> Any:
    if robot_id == "all":
        targets = list(state.registry.robots.keys())
    else:
        if robot_id not in state.registry.robots:
            return JSONResponse(
    ...
    for rid in targets:
        ...
        r = state.registry.robots.get(rid)
        if r:
            r.goal = None
            r.planned_path = []
    ...
    state.events.log("agent_stop", {"targets": targets})
...
async def post_robot_body(robot_id: str, request: Request) -> Any:
    ...
    msg: dict[str, Any] = {"type": "body_command", "action": action, **state.stamps()}
    if "height" in body:
        try:
    ...
    sent = await state.registry.send(robot_id, msg)
    state.events.log("agent_body", {"robot_id": robot_id, "action": action})
...
async def get_all_detections() -> Any:
    snap = state.review_store.snapshot()
    return {
        "tracks": list(state._detections.values()),
