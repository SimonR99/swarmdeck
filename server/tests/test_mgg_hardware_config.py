...
@pytest.mark.parametrize(
    "robot,filename,domain",
    [
        ("botman", "bunker", 17),
        ("aslan", "aslan_bunker", 49),
        ("spot", "spot", 0),
        ("asimov", "unitree_g1", 0),
    ],
)
def test_onboard_planner_matches_adapter(robot, filename, domain):
    compose = yaml.safe_load(
        (REPO / f"deploy/compose/docker-compose.robot-{robot}.yml").read_text()
    )
    mgg = compose["services"]["mgg"]
    cfg = yaml.safe_load(
        (REPO / f"adapters/adapter_ros2/config/{filename}.yaml").read_text()
    )
    ...
    assert (
        mgg["environment"]["ROS_DOMAIN_ID"]
        == f"${{{robot.upper()}_ROS_DOMAIN_ID:-{domain}}}"
    )
